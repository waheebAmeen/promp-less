      ...
      >
        <View className="max-w-4xl mx-auto w-full flex-1 px-4 md:px-6 pt-6 md:pt-10">
          ...
          <View className="flex-1">
            <Typography className="text-slate-400 mb-3 md:mb-4 font-medium px-2">{t('input.label', { defaultValue: 'What is on your mind?' })}</Typography>
            <View className="bg-surface/40 rounded-3xl md:rounded-4xl border border-white/5 shadow-glass overflow-hidden">
              <Input
                multiline
                numberOfLines={10}
              ...
              />
              <View className="flex-row justify-between items-center px-5 md:px-6 py-3 md:py-4 bg-white/5 border-t border-white/5">
                <View className="flex-row gap-2">
                   <Icon name="text" size={14} color="#475569" />
                   <Typography variant="caption" className="text-slate-500 font-bold text-xs md:text-sm">Smart Input</Typography>
                </View>
                <Typography variant="caption" className={`font-bold text-xs md:text-sm ${idea.length > 100 ? 'text-primary' : 'text-slate-500'}`}>
          ...
          <View className="pb-8 md:pb-10 pt-4 md:pt-6">
            <Button 
              ...
              className={`w-full h-14 md:h-16 rounded-2xl md:rounded-3xl ${idea.trim().length === 0 ? 'opacity-30 bg-surface border border-white/10' : 'bg-primary'}`}
            />
          </View>
        </View>
      </KeyboardAvoidingView>
