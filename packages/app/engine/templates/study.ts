export const studyPrompts = {
  ...
  bookSummarizer: {
    ...
    description: 'يقدم لك ملخصاً منظماً ودقيقاً للكتاب مع التركيز على الأفكار الرئيسية والأمثلة والتطبيقات العملية.',
    template: `Act as a professional book summarizer and comprehension expert.
Your task is to create a clear, accurate, and well-structured summary of the book: \${bookName}.
User's purpose for the summary:
\${summaryPurpose}
...
Reader's familiarity with the book:
\${readingStatus}
...
Instructions:
- Summarize only information that is supported by the provided book content or reliable information about the book.
- Do not invent chapters, concepts, examples, quotes, statistics, or claims.
- Identify the book's major topics, central ideas, important arguments, and practical lessons.
- For each major topic or concept, provide:
  1. Topic Overview
  2. Examples or explanations from the book
  3. Practical Application
  4. Key Takeaways
- Give greater emphasis to the user's selected focus areas.
- Adapt the depth and level of detail to the user's selected preferences.
- Use the requested output style consistently.
- If the exact book content is not available, clearly state that the summary is based only on the available information and do not fabricate missing content.
- If the user requests quotes, distinguish clearly between exact quotations and paraphrased ideas.
- Keep the final response around 800 words unless the selected summary depth or user's requirements clearly indicate otherwise.
- Format the response using Markdown with clear headings, subheadings, and bullet points where appropriate.
- Do not add unrelated information, decorative content, or instructions unrelated to summarizing the book.`,
    questions: [
      {
        id: 'bookName',
